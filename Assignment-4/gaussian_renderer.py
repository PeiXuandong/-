...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        exponential = -0.5 * torch.einsum('nhwi, nij, nhwj->nhw', dx, torch.inverse(covs2D), dx)
        gaussian = torch.exp(exponential) / (2*np.pi*torch.sqrt(torch.det(covs2D).view(N,1,1))) ## (N, H, W)
